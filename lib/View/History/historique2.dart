...
class _HistoriqueScreen extends State<HistoriqueScreen> {
  ...
  void _updateProductList(String searchTerm) async {
  // recherche est vide, affichez tous les produits
  if (searchTerm.isEmpty) {
    setState(() {
      _filteredProducts = List.from(_products);
      _totalProducts = _filteredProducts.length;
    });
  } else {
    // Filtrer les produits par nom ou code
    setState(() {
      _filteredProducts = _products.where((product) =>
          product.name.toLowerCase().contains(searchTerm.toLowerCase()) ||
          product.code.toLowerCase().contains(searchTerm.toLowerCase())
      ).toList();
      _totalProducts = _filteredProducts.length;
    });
  }
